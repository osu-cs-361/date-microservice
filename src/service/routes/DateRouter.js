...
const DateRouter = (pool) => {
  ...
  router.get("/datetime/interval/:interval_length", (req, res, next) => {
    try {
  ...
  router.get("/datetime/add/:duration_interval", (req, res, next) => {
    try {
  ...
  // ("amount" query param) of intervals ("duration_interval" URL param).
  router.get("/datetime/subtract/:duration_interval", (req, res, next) => {
    try {
      // Check duration interval exists and is valid
      if (!req.params.duration_interval) {
        throw new DateTimeError("No duration interval provided.");
  ...
  router.get("/events", authRoute(pool), async (_, res, next) => {
    try {
  ...
  router.post("/new-event", authRoute(pool), async (req, res, next) => {
    try {
  ...
  router.put("/edit-event/:id", authRoute(pool), async (req, res, next) => {
    try {
